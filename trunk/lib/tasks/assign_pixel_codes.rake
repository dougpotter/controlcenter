  ...
  task :ensure_redirect_configurations => :environment do
    ...
    for beacon_audience in Beacon.new.audiences
      ...
      for sync_rule in Beacon.new.sync_rules(beacon_audience["id"])
        ...
        if partner = Partner.new(:partner_code => beacon_audience.pid).find_apn
          partner_apn_id = partner["id"]
        else
          apn_sync_log.info("Beacon audience #{beacon_audience["id"]} and PID "+
            "#{beacon_audience.pid} was not found at Appnexus")
          next
        end
