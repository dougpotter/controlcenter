...
module Xgw
  ...
  class InputFields
    def initialize(allowed_keys, hash)
      ...
      if hash.nil?
        ...
        @hash = Utils.to_string_hash(hash)
      end
    end
    ...
    def merge(other_hash)
      @hash.to_hash.dup.update(Utils.to_string_hash(other_hash))
    end
  end
  ...
  class Output
    def initialize(hash=nil)
      hash = Utils.to_options_hash(hash || {})
    ...
    class << self
      ...
      def consume(name, options={}, &block)
        ...
        define_method("consume_#{name}") do
          ...
          input = InputFields.new(allowed_keys, Utils.to_indifferent_hash(workitem.fields['input'] || {}))
